...
do {
    ...
    if ($isDatabaseReady -eq $true) {
        ...
        $hostPort = [Environment]::GetEnvironmentVariable($environmentVariableStoringDockerHostPort, 'User')
        Write-Output "Docker host port used by $ContainerName container is: $hostPort"
        exit 0
    }
